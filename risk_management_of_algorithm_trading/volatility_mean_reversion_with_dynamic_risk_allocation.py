...
# Risk limits and increments to risk limits where we have good/bad months
...
for close_price in close:
    ...
    if (not risk_violated and
            ...
            abs (close_price - last_sell_price) > MIN_NUM_SHARES_PER_TRADE *stdev_factor)
            or
    ...
    # 2. We are short( -ve position ) and either APO trading signal value is at or below 0 or current position is profitable enough to lock profit.
          ...
          ((apo < APO_VALUE_FOR_BUY_ENTRY * stdev_factor and abs(
              close_price - last_buy_price) > MIN_PRICE_MOVE_FROM_LAST_TRADE * stdev_factor)
           or
           (position < 0 and (
        ...
        if position == 0:  # opening a new entry position
            position += num_shares_per_trade  # reduce position by the size of this trade
            buy_sum_price_qty += (close_price * num_shares_per_trade)  # update vwap sell-price
    ...
    # track trade-size/positions and risk limits as they evolve over time
