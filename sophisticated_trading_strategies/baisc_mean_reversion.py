...
NUM_PERIODS_FAST = 10 # static smoothing factor parameter for fast EMA
...
APO_VALUE_FOR_BUY_ENTRY = -10 # APO trading signal value below which to enter buy-orders
APO_VALUE_FOR_SELL_ENTRY = 10 # APO trading signal value above which to enter sell-orders
...
MIN_PROFIT_TO_CLOSE = 10 * NUM_SHARES_PER_TRADE # Minmum Open/Unrealized profit at which to close positions and lock profits
...
for close_price in close:
    ...
    # 1. The APO trading signal value is above sell-entry threshhold and the difference between last trade-price and current-price is different enough.
    ...
    if ((apo > APO_VALUE_FOR_SELL_ENTRY and abs(close_price - last_sell_price) > MIN_PRICE_MOVE_FROM_LAST_TRADE)# APO above sell entry threshold, we should sell
            or (position > 0 and (apo >= 0 or open_pnl > MIN_PROFIT_TO_CLOSE))): # long from -ve APO and APO has gone positive or position is profitable, sell to close position
...
# This section prepare the dataframe from the trading strategy results and visualizes the results
