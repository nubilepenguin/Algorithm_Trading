...
# Fetch daily data for 4 years, from 7 major currency pairs
...
delta_projected_actual_history = {} # history of differences between projected ClosePrice deviation
                                    # and actual ClosePrice deviation per currency pair
final_delta_projected_history = [] # history of differences between final projected ClosePrice deviation
                                    # for TRADING_INSTRUMENT and actual ClosePrice deviation
...
positions = [] # Container for tracking positions, +ve for long positions, -ve for short positions, 0 for flat position
...
buy_sum_qty = 0 # Summation of buy_trade_qty for very buy Trade made since last time being flat
...
sell_sum_qty = 0 # Summation of sell_trade_qty for very sell Trade made since last time being flat
open_pnl = 0 # Open/Unrealized PnL marked to market
closed_pnl = 0 # CLose/Realized PnL so far
...
NUM_SHARES_PER_TRADE = 1000000 # Number of shares to buy/sell on every trade
MIN_PROFIT_TO_CLOSE = 10  # Minmum Open/Unrealized profit at which to close positions and lock profits
...
for i in range(0, num_days):
    ...
    # also compute projected price deviation and find delta between projected and actual price deviation
    ...
    for symbol in SYMBOLS:
        ...
        if len(price_deviation_from_sma[symbol]) < 2: # need at least two observations to compute covariance/correlation
    ...
    # 1. The StatArb trading signal value is below Sell_Entry threshold and the difference between last trade-price and current-price is diffrent enough.
